extends Node
class_name SlideInFromBottomAnimation

## Animation that slides a node in from the bottom of the screen.
## Inverse of slide_down_animation.
...
## Animates a node sliding in from the bottom of the screen to its original y.
...
func animate(node: Node, original_y: float, on_complete: Callable = Callable()) -> Tween:
	...
	# Set starting position (off-screen to the bottom)
	...
	# Animate position y (or offset:y for CanvasLayer) from off-screen bottom to original position
