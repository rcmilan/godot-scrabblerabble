...
## Executes spin animations for tiles with EXTRA, MULTI, or EXPO modifiers.
## Tiles scale up and rotate 360 degrees, then return to normal.
...
func execute(tiles: Array[Tile], strategy: SpinTileAnimation) -> void:
	...
	for i in tiles.size():
		...
		# Phase 1: Spin up — scale up + start rotation
		...
		# Phase 2: Spin down — return scale
